-- ====================================================================
-- CityPulse Smart City Operations Platform — Initial Schema (Custom Schema: citypulse)
-- Migration Version: 001_initial_schema.sql
-- ====================================================================

-- 1. Create Isolated Custom Schema for CityPulse
CREATE SCHEMA IF NOT EXISTS citypulse;
SET search_path TO citypulse, public;
...
-- --------------------------------------------------------------------
-- Table: citypulse.users
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.users (
...
-- --------------------------------------------------------------------
-- Table: citypulse.districts
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.districts (
    id SERIAL PRIMARY KEY,
...
-- --------------------------------------------------------------------
-- Table: citypulse.metric_time_series
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.metric_time_series (
    id SERIAL PRIMARY KEY,
    timestamp TIMESTAMPTZ DEFAULT NOW(),
    district_id INTEGER REFERENCES citypulse.districts(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_metric_ts_timestamp ON citypulse.metric_time_series(timestamp);
CREATE INDEX IF NOT EXISTS idx_metric_ts_district ON citypulse.metric_time_series(district_id);
CREATE INDEX IF NOT EXISTS idx_metric_ts_domain ON citypulse.metric_time_series(domain);

-- --------------------------------------------------------------------
-- Table: citypulse.utilities_assets
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.utilities_assets (
    id SERIAL PRIMARY KEY,
    district_id INTEGER REFERENCES citypulse.districts(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
...
-- --------------------------------------------------------------------
-- Table: citypulse.traffic_corridors
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.traffic_corridors (
    id SERIAL PRIMARY KEY,
    district_id INTEGER REFERENCES citypulse.districts(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
...
-- --------------------------------------------------------------------
-- Table: citypulse.public_transit_vehicles
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.public_transit_vehicles (
    id SERIAL PRIMARY KEY,
    district_id INTEGER REFERENCES citypulse.districts(id) ON DELETE CASCADE,
...
-- --------------------------------------------------------------------
-- Table: citypulse.service_requests_311
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.service_requests_311 (
    id SERIAL PRIMARY KEY,
...
-- --------------------------------------------------------------------
-- Table: citypulse.emergency_units
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.emergency_units (
    id SERIAL PRIMARY KEY,
    district_id INTEGER REFERENCES citypulse.districts(id) ON DELETE CASCADE,
...
-- --------------------------------------------------------------------
-- Table: citypulse.infrastructure_assets
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.infrastructure_assets (
    id SERIAL PRIMARY KEY,
    district_id INTEGER REFERENCES citypulse.districts(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    asset_type VARCHAR(100) NOT NULL,
...
-- --------------------------------------------------------------------
-- Table: citypulse.alerts
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.alerts (
    id SERIAL PRIMARY KEY,
...
-- --------------------------------------------------------------------
-- Table: citypulse.maintenance_tickets
-- --------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS citypulse.maintenance_tickets (
    id SERIAL PRIMARY KEY,
...
ALTER TABLE citypulse.users ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.districts ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.metric_time_series ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.utilities_assets ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.traffic_corridors ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.public_transit_vehicles ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.service_requests_311 ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.emergency_units ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.infrastructure_assets ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.alerts ENABLE ROW LEVEL SECURITY;
ALTER TABLE citypulse.maintenance_tickets ENABLE ROW LEVEL SECURITY;

DO $$
DECLARE
    tbl text;
BEGIN
    FOR tbl IN SELECT tablename FROM pg_tables WHERE schemaname = 'citypulse' LOOP
        EXECUTE format('DROP POLICY IF EXISTS "Allow public read access" ON citypulse.%I', tbl);
        EXECUTE format('CREATE POLICY "Allow public read access" ON citypulse.%I FOR SELECT USING (true)', tbl);
        
        EXECUTE format('DROP POLICY IF EXISTS "Allow full access for service_role" ON citypulse.%I', tbl);
        EXECUTE format('CREATE POLICY "Allow full access for service_role" ON citypulse.%I FOR ALL USING (true) WITH CHECK (true)', tbl);
    END LOOP;
END $$;
...
-- 1. Seed Districts
INSERT INTO citypulse.districts (code, name, population, lat, lng, area_sq_km, bounds_json)
VALUES
...
INSERT INTO citypulse.utilities_assets (district_id, name, asset_type, status, electricity_mw, water_pressure_psi, water_flow_gpm, gas_pressure_bar, waste_fill_pct)
VALUES
...
INSERT INTO citypulse.traffic_corridors (district_id, name, start_location, end_location, speed_mph, flow_veh_hr, congestion_index, incident_active)
VALUES
...
INSERT INTO citypulse.public_transit_vehicles (district_id, route_name, vehicle_type, vehicle_code, delay_minutes, ridership_count, health_score, status)
VALUES
...
INSERT INTO citypulse.infrastructure_assets (district_id, name, asset_type, location_description, lat, lng, condition_score, risk_level, estimated_days_to_failure, maintenance_status)
VALUES
...
INSERT INTO citypulse.alerts (alert_code, domain, district_id, severity, title, description, root_cause_hint)
VALUES
...
INSERT INTO citypulse.emergency_units (district_id, unit_code, unit_type, status, avg_response_time_min, active_incidents_count)
VALUES
...
INSERT INTO citypulse.service_requests_311 (request_number, title, category, description, district_id, lat, lng, priority, status, sla_hours)
VALUES
